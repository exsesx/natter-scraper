...
The actual CLI adds flag validation, terminal handling, and signals. With `--input`, it calls `readCatalog` instead of `crawl`, validates the saved JSON before opening the terminal or writing output, and skips all network and Chrome work. Loaded catalogs use the same export and browsing paths, with no invented crawl count or duration.

For a fresh scrape, [`crawl`](../src/crawl.ts) returns `Effect<CrawlResult, CrawlError>` after bounded discovery and product observation. Listing discovery completes one bounded batch before starting the next. Product work uses one `Effect.forEach` over the discovered URLs, with bounded concurrency, so finishing a product immediately frees its slot for the next URL. The default is one to six slots, chosen from available CPUs and total RAM; an explicit `--concurrency` overrides it. Each slot includes the document fetch and browser observations, while Chrome may issue additional resource requests.

[`requestHtml`](../src/http.ts) owns each fetch attempt. Its timeout covers redirects and body reads; retries wait for the longer of exponential backoff and `Retry-After`. The crawl deadline includes retry waits. Finalizer cleanup can extend elapsed time beyond either deadline.

Expected failures use yieldable `Data.TaggedError` types. `RequestFailure` retains the URL, attempt count, and underlying `HTTP`, `Transport`, `RequestTimeout`, or `InvalidResponse` error. Snapshot parsing and browser observation use `ExtractionError`; catalog validation uses `CatalogError`. Saved-file reads, JSON parsing, and imported-data validation use `InputError`. Output failures use `OutputError`. Unexpected exceptions remain defects. In particular, a synchronously throwing injected fetch adapter is not retried as a transport failure.

## Failure and cleanup

Each attempt owns an abort controller. The response scope owns both the response and any acquired body reader. Redirect responses close before the next request, and interruption waits for pending fetch and body cleanup. A failed request or product observation interrupts its siblings and waits for their finalizers.
...
sequenceDiagram
    ...
    participant Request as HTTP attempt<br/>and response scope
    participant Native as Fetch or<br/>body reader
    Run->>Request: Interrupt active request
    Request->>Native: Abort network work
    alt Fetch still pending
        Request->>Native: Await fetch settlement and cancel returned body
    else Body reader acquired
        Request->>Native: Await reader cancellation and release lock
    end
    Native-->>Request: Cleanup settles
    Request-->>Run: Finish interruption after finalizers
```
...
Keep snapshot parsing in `site.ts`, option interaction in `product-browser.ts`, catalog rules in `catalog.ts`, saved-file validation in `input.ts`, and pure serialization in `format.ts`. The browser promise adapter runs the pure parsing Effect when it receives settled markup; other runtime entry points are CLI startup, tests, and React callbacks. HTTP and file adapters remain native to preserve explicit redirect validation, awaited fetch cancellation, and typed file-close failures. Preserve those behaviors if replacing them with platform services.

Tests use `Effect.runPromise` for values and `Effect.runPromiseExit` for errors or defects. [Crawl tests](../tests/crawl.test.ts) cover retries, deadlines, sibling interruption, and delayed cleanup; [output tests](../tests/output.test.ts) cover file preservation and blocked writes. [Input tests](../tests/input.test.ts) cover saved JSON shape, exact cents, and total validation. Run `bun run check`. UI or signal changes also require [terminal verification](../README.md#develop-and-verify); [standalone verification](distribution.md#verify-on-the-target-platform) exercises the same entry point after compilation.
